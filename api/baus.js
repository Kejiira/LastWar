...
export default async function handler(req, res) {
  const token  = process.env.GITHUB_TOKEN;     // PAT com "Contents: Read/Write" pro repo alvo
  ...
  if (req.method === 'POST') {
    ...
    if (!Array.isArray(body)) return res.status(400).json({error: 'Payload deve ser um array'});

    // pega sha atual (se já existir)
    let sha;
    {
      const r = await fetch(`${baseUrl}?ref=${branch}`, {
        headers: { Authorization: `Bearer ${token}`, 'Accept': 'application/vnd.github+json' }
    ...
    if (!commitRes.ok) return res.status(commitRes.status).json({error:'Falha ao salvar no GitHub', detail: await commitRes.text()});
